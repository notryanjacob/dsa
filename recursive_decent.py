class RecursiveDescentParser:
    ...
    def parse_non_terminal(self, non_terminal):
        print(f"Entering {non_terminal}() - Karan Gautam reg. No.: 22BCE2834")

        for rule in self.grammar[non_terminal]:
            saved_position = self.position
            if self.match_rule(rule):
                print(f"Exiting {non_terminal}() with success - Karan Gautam reg. No.: 22BCE2834")
                return True
            self.position = saved_position
        
        print(f"Exiting {non_terminal}() with failure - Karan Gautam reg. No.: 22BCE2834")
        return False
...
if __name__ == "__main__":
    ...
    if parser.parse(input_string):
        print("\nString is accepted. - Karan Gautam reg. No.: 22BCE2834")
    else:
        print("\nString is not accepted. - Karan Gautam reg. No.: 22BCE2834")